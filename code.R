...
#i. Count of members and casual riders
halfyearly %>% 
    group_by(member_casual) %>% 
    summarise(ride_count = length(ride_id), ride_percentage = (length(ride_id) / nrow(halfyearly)) * 100)
ggplot(halfyearly, aes(x = member_casual, fill=member_casual)) +
    geom_bar() +
    labs(x="User type", y="Number Of Rides", title= "Casuals vs Members distribution")

#ii. Visualizing the average time taken and average distance covered by user type
...
#iii. Visualizing number of rides in a week by user type
halfyearly %>% 
  mutate(weekday=wday(started_at, label= TRUE)) %>% 
  group_by(member_casual,weekday) %>% 
...
#iv. Visualizing the average duration taken by user type in a week
halfyearly %>% 
  mutate(weekday=wday(started_at, label= TRUE)) %>% 
  group_by(member_casual,weekday) %>% 
  summarise(number_of_rides= n(),avg_duration=mean(ride_length),.groups="drop") %>% 
...
#v. Visualizing the number of rides by user type across months
halfyearly %>% 
...
#vi. Visualizing the average duration of rides by user type across months
halfyearly %>% 
  mutate(month=month(started_at, label= TRUE)) %>% 
  group_by(member_casual,month) %>% 
  summarise(number_of_rides= n(),avg_duration=mean(ride_length),.groups="drop") %>% 
...
#vii. Visualizing the bike demand by hour in a day
halfyearly %>%
...
#viii. Visualizing the bike demand by hour across the week
halfyearly %>%
mutate(weekday=wday(started_at, label= TRUE)) %>% 
    ggplot(aes(time, fill=member_casual))+
...
#ii. Visualizing the bike type used by the users during the week
bike_type %>%
    mutate(weekday = wday(started_at, label = TRUE))%>%
...
#iii. Visualizing the bike type used by the users across months
bike_type %>% 
  mutate(month=month(started_at, label= TRUE)) %>% 
